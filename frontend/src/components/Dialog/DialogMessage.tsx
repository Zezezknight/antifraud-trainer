...
function DialogMessage({ type, text, typing = false }: DialogMessageProps) {
  ...
  return (
    ...
    >
      <span
        className={`text-sm sm:text-base max-w-3/4 sm:max-w-2/3 md:max-w-1/2 inline-block rounded-lg border px-4 py-3 ${isOpponent ? 'bg-background  border-border' : 'bg-primary-subtle border-primary'}`}
      >
        {typing ? (
          <Ellipsis
